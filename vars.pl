...
use Irssi::TextUI;
    ...
    my $user = getpwuid($<);
    $config{'vardata_path'} = '/home/' . $user . '/.irssi/';
    ...
    move('/home/' . $user . '/.irssi/scripts/varspl/.vardata', $config{'vardata_path'} . '.vardata') if -e '/home/' . $user . '/.irssi/scripts/varspl/.vardata';
    File::Path::rmtree('/home/' . $user . '/.irssi/scripts/varspl');
...
sub cmd_editvar {
    ...
    if(scalar(@args) < 2) {
        Irssi::print('Syntax Error: ' . (scalar(@args) == 1 ? 'Single' : 'No') . ' argument given.');
        Irssi::print('Type "/help vars editvar" for command usage');
        return;
    }
    else {
        $farg = shift @args;
...
sub cmd_cpvar {
    ...
    if(scalar(@args) < 2 || scalar(@args) > 3) {
        ...
        Irssi::print('Type "/help vars cpvar" for command usage');
        return;
    }
    ...
    if($var =~ /-(\w+)/) {
        ...
        foreach my $a (@flags) {
            if($a !~ /^[pf]$/) {
                Irssi::print("Error: '$a' is not a valid flag.");
                Irssi::print('Type "/help vars cpvar" for command usage');
                return;
            }
            unless ($seen{$a} || $a !~ /^[pf]$/) {
    ...
    if($foo{$newvar}) {
        unless($flags[0] eq 'f') {
            ...
            if($safe eq 'ERROR') {
                ...
                $foo{$newvar} = ($flags[1] || $flags[0] eq 'p') ? $safe : $foo{$var};
            }
            Irssi::print("Contents of " . (($flags[1] || $flags[0] eq 'p') ? 'fully expanded' : '')  . " variable '$var' successfully copied to existing variable '$newvar'.");
            store(\%foo, $config{'vardata_path'}.'.vardata');
        }
    }
    else {
        ...
        if($safe eq 'ERROR') {
            ...
            $foo{$newvar} = ($flags[1] || ($flags[0] && $flags[0] eq 'p')) ? $safe : $foo{$var};
        }
        Irssi::print("Contents of fully expanded variable '$var' successfully copied to new variable '$newvar'.");
        store(\%foo, $config{'vardata_path'}.'.vardata');
    }
...
sub cmd_varlist {
    ...
    Irssi::print('=' x 49, MSGLEVEL_CLIENTCRAP);
    foreach my $key (sort {lc($a) cmp lc($b)} keys %foo) {
        Irssi::print("\x038\x02\x02$key\x03" . ': \'' . $foo{$key} . '\'' . (loopcheck($foo{$key}) ne $foo{$key} ? (" - ('\x033\x02\x02" . loopcheck($foo{$key}) . "\x03')") : ""), MSGLEVEL_CLIENTCRAP);
    }
    Irssi::print('', MSGLEVEL_CLIENTCRAP);
}
...
sub cmd_undo {
    my ($repeat) = @_;
    unless($repeat =~ /^(((\s*)(\d+)(\s*))|\s*)$/) {
        Irssi::print('Error: Non-integer argument.');
        return;
    }
    if(!@undo) {
    ...
    if($4 && $repeat > scalar(@undo)) {
        $repeat = scalar(@undo) - 1;
    }
    elsif($4) {
        $repeat = $4 - 1;
    }
...
sub cmd_redo {
    ...
    if($4 && $repeat > scalar(@redo)) {
        $repeat = scalar(@redo) - 1;
    }
    elsif($4) {
        $repeat = $4 - 1;
    }
...
sub cmd_help {
    my $help = "For help on a specific command, e.g. /mkvar, type /help varspl mkvar.\n"
             . "This help is also available online at: http://users.aber.ac.uk/ord8/tech/non-web/varspl/?commands";
    if ($_[0] eq 'varspl') {
...
sub help_cpvar {
    my @help = (
                     "",
                     "Synopsis: /cpvar [-fp] variable name",
                     "",
                    ...
                    ."depend on 'foo'.",
                     ...
                     "",
                     "Adding the -p flag (p for 'preserve') will allow one to copy the inflated values of a "
...
sub help_undo {
    my @help = (
                    ...
                    ."through vars.pl services. Undoing an action will create an item in the redo buffer (see /redo).",
                     "",
                     "The command takes an optional integer argument, which will specify the number of steps to undo.",
                     "",
                );
    return @help;
}
sub help_redo {
    my @help = (
                    ...
                    ."the redo buffer is compeltely cleared out.",
                     "",
                     "The command takes an optional integer argument, to specify number of steps to redo.",
                     "",
                );
    return @help;
}
